...
```
docker build -t deskobj/ansible-playbook .
```

### Test

```
$ docker run --name ansible-playbook --rm deskobj/ansible-playbook --version
...
```
docker run --rm -it -v PATH_TO_LOCAL_PLAYBOOKS_DIR:/ansible/playbooks deskobj/ansible-playbook PLAYBOOK_FILE
```
...
```
docker run --rm -it -v $(pwd):/ansible/playbooks deskobj/ansible-playbook site.yml
```
...
docker run --rm -it \
    ...
    -v ~/.ssh/id_rsa.pub:/root/.ssh/id_rsa.pub \
    -v $(pwd):/ansible/playbooks \
    deskobj/ansible-playbook site.yml
```
...
docker run --rm -it -v $(pwd):/ansible/playbooks --entrypoint ansible-vault \
  deskobj/ansible-playbook encrypt FILENAME
```
